...
def make_feat(train, test):
    ...
    data = data.fillna(data.mean(axis=0))   #缺失值用平均值填充
...
test_preds = np.zeros((test_feat.shape[0], 8))    # 0代表在行上填充0,5代表5列 训练5次得到5中不同的结果，求平均
...
submission.to_csv(r'../sub/sub{}.csv'.format(datetime.datetime.now().strftime('%Y%m%d_%H%M%S')), header=None,
