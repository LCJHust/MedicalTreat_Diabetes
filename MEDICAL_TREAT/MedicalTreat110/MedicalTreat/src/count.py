...
def mean_value():
    ...
    train_data = train_data.fillna(0)   #缺失值填充0
...
def data_corr():
    ...
    del train_data['Sex']
    del train_data['Date']
    corr = train_data.corr()   #以DataFrame的形式返回完整的相关系数
    print(0)


def compare():
    sub0 = pd.read_csv('../sub/sub8383.csv')
    sub0.columns = ['pred0']
    sub1 = pd.read_csv('../sub/sub8506.csv')
    sub1.columns = ['pred1']
    result = pd.concat([sub0, sub1], axis=1)
    print (result['pred0'].mean(), result['pred0'].std())  #平均值，标准差
