...
train = pd.read_csv(train_data_file, encoding='gb2312')
test = pd.read_csv(test_data_file, encoding='gb2312')
...
def xgboost_make(train_feat, test_feat):  # xgboost训练及线下测评
    ...
    predictors = [f for f in test_feat.columns if f not in ['id', 'Blood_Sugar']] #除去id，Blood_Sugar之外，其他特征用于预测
    ...
    for i, (train_index, test_index) in enumerate(kf):    #enumerate多用于在for循环中得到计数
        print('第{}次训练...'.format(i+1))
        train_feat1 = train_feat.iloc[train_index]
        train_feat2 = train_feat.iloc[test_index]
        xgb_train1 = xgb.DMatrix(train_feat1[predictors], train_feat1['Blood_Sugar'])    #训练集预测血糖值
        xgb_train2 = xgb.DMatrix(train_feat2[predictors], train_feat2['Blood_Sugar'])    #测试集预测血糖值
        ...
        df = df.sort_values('fscore', ascending=False)     #特征的贡献程度排名
    ...
    submission.to_csv('../sub/sub_xgb0.csv', index=False)      #xgboost模型预测结果
